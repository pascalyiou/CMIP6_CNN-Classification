...
l.Apred=list()
for(seas in l.seas){
    fname=paste("proba-class-v1_CMIP6-other_",seas,"_",nmod,"_",jobid,sep="")
...
fout=paste("proba-class-v1_CMIP6-other_",nmod,"_",jobid,".pdf",
           sep="")
...
for(seas in l.seas){
    ...
    axis(side=2,at=1:ncol(l.Apred[[seas]]),labels=l.names.test,las=2)
    axis(side=1,at=1:nrow(l.Apred[[seas]]),labels=l.names.ref,las=2)
    box()
    i=i+1
}
dev.off()

...
## Needs to have run:
## R CMD BATCH "--args SAISON NMOD JOBID" ${HOME}/programmes/RStat/CMIP6class/CMIP6_tensorflow-verif-other_v1.R
## for the four seasons
## Pascal Yiou (LSCE), Jan. 2025
## Need to: module load R/4.4.1
## Se lance par:
## R CMD BATCH "--args NMOD SCEN YR1 YR2 JOBID" ${HOME}/programmes/RStat/CMIP6class/CMIP6_verif-scen_plots.R
##
SI=Sys.info()
user=SI[["user"]]
if(SI[[1]] == "Linux"){
...
if(length(args)>0){
    ...
    scen = args[i];i=i+1 ## Scenario IPCC
    yr1 = as.numeric(args[i]) ;i=i+1
    yr2 = as.numeric(args[i]) ;i=i+1
...
l.Apred=list()
for(seas in l.seas){
    fname=paste("proba-class-v1_CMIP6_",scen,"_",yr1,"-",yr2,"_",seas,"_",nmod,"_",jobid,sep="")
...
fout=paste("proba-class-v1_CMIP6-scen_",scen,"_",yr1,"_",yr2,"_",
           nmod,"_",jobid,".pdf",
           sep="")
...
for(seas in l.seas){
    ...
    axis(side=2,at=1:ncol(l.Apred[[seas]]),labels=l.mod.scen,las=2)
